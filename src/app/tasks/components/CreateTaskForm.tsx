...
interface FormikSuggestionLogicProps {
  users: User[]
  setSuggestedAssignment: React.Dispatch<React.SetStateAction<{ userId: string; durationDays: number } | null>>
...
const FormikSuggestionLogic: React.FC<FormikSuggestionLogicProps> = ({
  ...
  useEffect(() => {
    ...
    if (suggestedAssignment) {
      ...
      if (values.assignedUserIds.length === 0 || values.assignedUserIds[0] !== suggestedAssignment.userId) {
  ...
  }, [suggestedAssignment, fetchingSuggestion, values.brandId, values.categoryId, setFieldValue, values.assignedUserIds.length, setSuggestedAssignment, setFetchingSuggestion, values.assignedUserIds])
...
export const CreateTaskForm: React.FC = () => {
  ...
  const [assignedUsersDisplay, setAssignedUsersDisplay] = useState<User[]>([])

  // ✅ ESTADOS PARA LA SUGERENCIA EN EL COMPONENTE PADRE
  const [suggestedAssignment, setSuggestedAssignment] = useState<{ userId: string; durationDays: number } | null>(null)
  const [fetchingSuggestion, setFetchingSuggestion] = useState(false)

  // ✅ CÁLCULO DE suggestedUser A PARTIR DEL ESTADO DEL PADRE
  const suggestedUser = suggestedAssignment ? users.find(u => u.id === suggestedAssignment.userId) : null
  ...
  const handleSubmit = async (values: FormValues) => {
    try {
      ...
      const finalDurationDays = parseFloat(values.durationDays as string) // Asegurar que sea string para parseFloat
      ...
      const payload = {
        ...
        assignedUserIds: values.assignedUserIds.length > 0 ? values.assignedUserIds : undefined,
      ...
      setAssignedUsersDisplay(createdTask.assignees.map((a: any) => a.user))

      setLoading(false)

      const assignedUserNames = createdTask.assignees?.map((a: any) => a.user.name).join(', ') ?? 'alguien'
      const brandName = createdTask.brand?.name ?? 'Brand desconocido'

      const startDate = new Date(createdTask.startDate).toLocaleDateString('es-PE', {
        year: 'numeric', month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit'
      })
      const endDate = new Date(createdTask.deadline).toLocaleDateString('es-PE', {
        year: 'numeric', month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit'
      })
            ...
            return (
              <Form className="space-y-4">
                ...
                <UserAssignmentSelect
                  ...
                  error={Array.isArray(errors.assignedUserIds) ? errors.assignedUserIds.join(', ') : errors.assignedUserIds}
                />

                {assignedUsersDisplay.length > 0 && (
                  <Typography level="body-sm">
                    Asignado(s) a: {assignedUsersDisplay.map(u => u.name).join(', ')}
                  </Typography>
                )}
                ...
                <Button
                  type="submit"
                  fullWidth
                  disabled={loading || brands.length === 0 || fetchingSuggestion}
                >
